...
public class Slf4jSpyLogDelegator implements SpyLogDelegator {
	 ...
	 */
	private final Logger jdbcLogger = Logger.getLogger("jdbc.audit");

	/**
	 * Logger that shows JDBC calls for ResultSet operations
	 */
	private final Logger resultSetLogger = Logger.getLogger("jdbc.resultset");

	/**
	 * Logger that shows only the SQL that is occuring
	 */
	private final Logger sqlOnlyLogger = Logger.getLogger("jdbc.sqlonly");

	/**
	 * Logger that shows the SQL timing, post execution
	 */
	private final Logger sqlTimingLogger = Logger.getLogger("jdbc.sqltiming");

	/**
	 * Logger that shows connection open and close events as well as current number
	 * of open connections.
	 */
	private final Logger connectionLogger = Logger.getLogger("jdbc.connection");
	 ...
	 */
	private final Logger debugLogger = Logger.getLogger("log4jdbc.debug");
	 ...
	 * @param execTime
	 *            optional amount of time that passed before an exception was thrown
	 *            when sql was being executed. caller should pass -1 if not used
	 */
	public void exceptionOccured(Spy spy, String methodCall, Exception e, String sql, long execTime) {
		...
		if (sql == null) {
			...
			if (sqlTimingLogger.isDebugEnabled()) {
				sqlTimingLogger
						.error(getDebugInfo() + nl + spyNo + ". " + sql + " {FAILED after " + execTime + " msec}", e);
			} else {
	 ...
	 * @param methodCall
	 *            a description of the name and call parameters of the method that
	 *            returned.
	 * @param returnMsg
	 *            return value converted to a String for integral types, or String
	 *            representation for Object. Return types this will be null for void
	 *            return types.
	 */
	...
	public void sqlTimingOccured(Spy spy, long execTime, String methodCall, String sql) {
		if (sqlTimingLogger.isDebugEnabled() && (!DriverSpy.DumpSqlFilteringOn || shouldSqlBeLogged(sql))) {
	...
	private String buildSqlTimingDump(Spy spy, long execTime, String methodCall, String sql, boolean debugInfo) {
		...
		SqlLogsDatas sqlentitys= ExtAjaxOfJsService.sqlthreadlocat.get();
		if(sqlentitys== null) {
			sqlentitys = new SqlLogsDatas();
			sqlentitys.setThreadid(Thread.currentThread().getName());
			sqlentitys.setUserid("SYSTEM");
		}
		...
		sql = processSql(sql);
		if(sql.indexOf("tx_sys_sqllogs")==-1) {
			sqlentitys.setSqltiming(execTime);
			sqlentitys.setRealsql(sql);
			Map<String,Object> sqlparame = JSON.parseObject(JSON.toJSONString(sqlentitys),new TypeReference<Map<String,Object>>(){});
			sqlparame.put("id", StringUtils.getUUID());
			sqlparame.put("createdate", new Date());
			TxSessionFactory t =SpringContextUtil.getApplicationContext().getBean(TxSessionFactory.class);
			try {
				t.getTxSession().create("tx_sys_sqllogs", sqlparame);
	 ...
	 * version that prints the stack trace information from the point just before we
	 * got it (net.sf.log4jdbc)
	 *
	...
	private static String getDebugInfo() {
		...
		if (stackTrace != null) {
			...
			/**
			 * The DumpFullDebugStackTrace option is useful in some situations when we want
			 * to see the full stack trace in the debug info- watch out though as this will
			 * make the logs HUGE!
			 */
			if (DriverSpy.DumpFullDebugStackTrace) {
				...
				if (j == 0) // if app not found, then use whoever was the last guy that called a log4jdbc
							// class.
				{
